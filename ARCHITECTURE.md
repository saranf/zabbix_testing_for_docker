# 🏗️ 아키텍처 문서

## 시스템 구성도

```
Internet
    ↓
[Port 80/443]
    ↓
┌─────────────────────────────────────────┐
│  zabbix-reverse-proxy (Nginx)          │
│  - HTTPS 강제                           │
│  - 보안 헤더                            │
│  - Rate Limiting                        │
│  - SSL/TLS 1.2+                        │
└─────────────────────────────────────────┘
    ↓ (내부 네트워크)
┌─────────────────────────────────────────┐
│  zabbix-web (Zabbix Web UI)            │
│  - PHP + Nginx                          │
│  - Port 8080 (내부만)                   │
└─────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────┐
│  zabbix-server (Zabbix Server)         │
│  - Port 10051 (내부)                    │
│  - Port 10847 (외부 - Agent 통신)      │
└─────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────┐
│  postgres-server (PostgreSQL)           │
│  - Port 5432 (내부만)                   │
│  - 데이터 영구 저장                     │
└─────────────────────────────────────────┘

┌─────────────────────────────────────────┐
│  certbot (SSL 인증서 관리)              │
│  - 자동 발급                            │
│  - 자동 갱신 (12시간마다 체크)          │
└─────────────────────────────────────────┘

┌─────────────────────────────────────────┐
│  zabbix-agent (자체 모니터링)           │
│  - Zabbix 서버 자체 모니터링            │
└─────────────────────────────────────────┘
```

---

## Docker 네트워크 구성

### zabbix-network (Bridge)

모든 컨테이너는 격리된 Docker 네트워크에서 통신합니다.

**내부 통신만 허용**:
- `postgres-server:5432` - 데이터베이스
- `zabbix-web:8080` - 웹 UI
- `zabbix-server:10051` - 서버 내부 포트

**외부 노출 포트** (`.env`에서 변경 가능):
- `${HTTP_PORT:-80}` - HTTP (HTTPS로 리다이렉트)
- `${HTTPS_PORT:-443}` - HTTPS (Nginx Reverse Proxy)
- `${ZABBIX_SERVER_PORT:-10847}` - Zabbix Server (Agent 통신용)
- `${SSH_PORT:-22}` - SSH (방화벽 설정용)

---

## 포트 설정

모든 포트는 `.env` 파일에서 중앙 관리됩니다:

```env
# 포트 설정 (원하는 포트로 변경 가능)
HTTP_PORT=80
HTTPS_PORT=443
ZABBIX_SERVER_PORT=10847
SSH_PORT=22
```

**포트 변경 방법**:
1. `.env` 파일에서 원하는 포트로 변경
2. `docker-compose down && docker-compose up -d` 실행
3. 방화벽 규칙이 자동으로 새 포트에 맞춰 적용됨

**주의사항**:
- 포트 변경 시 DNS 설정이나 외부 방화벽도 함께 변경 필요
- 1024 이하 포트는 root 권한 필요
- 다른 서비스와 포트 충돌 주의

---

## 컨테이너 상세 정보

### 1. zabbix-reverse-proxy (Nginx)

**이미지**: `nginx:alpine`  
**역할**: 리버스 프록시 및 보안 게이트웨이

**주요 기능**:
- HTTPS 강제 (HTTP → HTTPS 리다이렉트)
- SSL/TLS 1.2+ 암호화
- 보안 헤더 추가 (HSTS, CSP, X-Frame-Options 등)
- Rate Limiting (DDoS 방지)
- 로그인 페이지 추가 보호
- 정적 파일 캐싱
- 민감한 파일 접근 차단

**볼륨**:
- `./nginx/nginx.conf` → `/etc/nginx/nginx.conf` (읽기 전용)
- `./nginx/conf.d` → `/etc/nginx/conf.d` (읽기 전용)
- `./certbot/conf` → `/etc/letsencrypt` (SSL 인증서)
- `nginx-logs` → `/var/log/nginx` (로그)

---

### 2. zabbix-web (Zabbix Web UI)

**이미지**: `zabbix/zabbix-web-nginx-pgsql:alpine-6.4-latest`  
**역할**: Zabbix 웹 인터페이스

**주요 기능**:
- PHP 기반 웹 UI
- PostgreSQL 연결
- 내부 Nginx 서버 (포트 8080)

**환경 변수**:
- `ZBX_SERVER_HOST`: zabbix-server
- `DB_SERVER_HOST`: postgres-server
- `PHP_TZ`: Asia/Seoul

---

### 3. zabbix-server (Zabbix Server)

**이미지**: `zabbix/zabbix-server-pgsql:alpine-6.4-latest`  
**역할**: 모니터링 서버 코어

**주요 기능**:
- 데이터 수집 및 처리
- 트리거 평가
- 알림 발송
- SNMP Trap 지원

**포트**:
- `10051` (내부) - 웹 UI 통신
- `10847` (외부) - Agent 통신

---

### 4. postgres-server (PostgreSQL)

**이미지**: `postgres:15-alpine`  
**역할**: 데이터베이스

**주요 기능**:
- Zabbix 데이터 저장
- 설정 정보 저장
- 히스토리 데이터 저장

**보안**:
- 내부 네트워크만 접근 가능
- 외부 포트 노출 없음

**볼륨**:
- `postgres-data` - 데이터 영구 저장

---

### 5. certbot (SSL 인증서 관리)

**이미지**: 커스텀 빌드 (certbot/certbot:latest 기반)
**역할**: SSL 인증서 자동 관리

**주요 기능**:
- Let's Encrypt 인증서 발급
- **크론탭 내장** - 매일 오전 2시 자동 갱신 체크
- 만료 30일 전 자동 갱신
- Nginx 자동 재시작 (인증서 갱신 시)

**크론탭 설정**:
```
0 2 * * * /usr/local/bin/renew-certs.sh
```

**볼륨**:
- `./certbot/conf` - 인증서 저장
- `./certbot/www` - ACME Challenge
- `/var/run/docker.sock` - Nginx 재시작용 (선택사항)

---

### 6. zabbix-agent (Zabbix Agent)

**이미지**: `zabbix/zabbix-agent:alpine-6.4-latest`  
**역할**: Zabbix 서버 자체 모니터링

**주요 기능**:
- Zabbix 서버 리소스 모니터링
- CPU, 메모리, 디스크 사용량 추적

---

## 데이터 볼륨

| 볼륨 이름 | 용도 | 크기 예상 |
|-----------|------|-----------|
| `postgres-data` | PostgreSQL 데이터 | 증가 (히스토리 데이터) |
| `zabbix-server-data` | Zabbix 서버 데이터 | 소량 |
| `zabbix-web-data` | 웹 UI 데이터 | 소량 |
| `zabbix-snmptraps` | SNMP Trap 데이터 | 소량 |
| `nginx-logs` | Nginx 로그 | 증가 (로그 로테이션 권장) |

---

## 보안 계층

### Layer 1: 네트워크 보안
- Docker 네트워크 격리
- 최소 포트 노출
- 내부 서비스 외부 접근 차단

### Layer 2: SSL/TLS 보안
- TLS 1.2+ 강제
- 강력한 암호화 스위트
- OCSP Stapling
- Perfect Forward Secrecy

### Layer 3: 애플리케이션 보안
- 보안 헤더 (HSTS, CSP, X-Frame-Options 등)
- Rate Limiting
- 파일 접근 제어
- 서버 정보 숨김

### Layer 4: 데이터 보안
- 데이터베이스 비밀번호 보호
- 환경 변수 분리 (.env)
- 읽기 전용 설정 파일

---

## 성능 최적화

### Nginx
- Gzip 압축 활성화
- 정적 파일 캐싱 (7일)
- Keepalive 연결
- 버퍼 최적화

### PostgreSQL
- 연결 풀링
- 인덱스 최적화 (Zabbix 기본 제공)

### Zabbix
- Poller 프로세스 최적화
- 히스토리 데이터 보관 기간 설정

---

## 확장성

### 수평 확장
- Zabbix Proxy 추가 가능
- 데이터베이스 복제 가능
- Nginx 로드 밸런싱 가능

### 수직 확장
- 컨테이너 리소스 제한 조정
- PostgreSQL 메모리 설정 조정
- Zabbix 프로세스 수 증가

---

## 모니터링 및 로깅

### 로그 위치
- Nginx: `docker-compose logs zabbix-reverse-proxy`
- Zabbix Server: `docker-compose logs zabbix-server`
- Zabbix Web: `docker-compose logs zabbix-web`
- PostgreSQL: `docker-compose logs postgres-server`

### Health Check
모든 컨테이너에 헬스체크 설정:
- 자동 복구
- 상태 모니터링
- 의존성 관리

---

## 백업 전략

### 데이터베이스 백업
```bash
docker-compose exec postgres-server pg_dump -U zabbix zabbix > backup.sql
```

### 설정 백업
```bash
tar -czf backup.tar.gz docker-compose.yml .env nginx/ certbot/
```

### 복원
```bash
docker-compose exec -T postgres-server psql -U zabbix zabbix < backup.sql
```

---

## 업그레이드 절차

1. 백업 수행
2. 이미지 업데이트
   ```bash
   docker-compose pull
   ```
3. 컨테이너 재시작
   ```bash
   docker-compose down
   docker-compose up -d
   ```
4. 로그 확인
   ```bash
   docker-compose logs -f
   ```

---

## 문제 해결

### 컨테이너 재시작
```bash
docker-compose restart [service-name]
```

### 로그 확인
```bash
docker-compose logs -f [service-name]
```

### 완전 재설치
```bash
docker-compose down -v
docker-compose up -d
```

---

**더 자세한 정보**:
- [README.md](README.md) - 설치 가이드
- [SECURITY.md](SECURITY.md) - 보안 설정
- [QUICKSTART.md](QUICKSTART.md) - 빠른 시작

